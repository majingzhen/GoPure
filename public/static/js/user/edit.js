layui.use(['form', 'layer'], function(){
    ...
    var editRoleSelect; // 保存xmSelect实例
    ...
    form.on('submit(userEditForm)', function(data){
        ...
        // 获取选中的角色ID
        //var roleIds = editRoleSelect.getValue();
        console.log(editRoleSelect.getValue())
        //formData.roleIds = roleIds;
        formData.roleIds = Convert.toIntArray(formData.roleIds)
    ...
    // 加载角色列表和用户数据
    async function initRolesAndUserData() {
        // 再获取用户数据
        const userRes = await request.get('/user/get', { id: userId });
        var options = {
            el: '#editRoleSelect',
            name: 'roleIds',
            layVerify: 'required',
